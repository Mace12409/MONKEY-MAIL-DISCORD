...
async function modmail(client, options = []) {
  ...
  // Handle the interaction for the confirmation prompt
  client.on("interactionCreate", async (interaction) => {
    if (interaction.isButton()) {
      if (interaction.customId === "confirm_create_ticket") {
        ...
        async function modmail(client, options = []) {
          ...
          client.on("messageCreate", async (message) => {
            ...
            if (message.channel.type === "DM") {
              ...
              let usersChannel = await guild.channels.cache.find(
              ...
              if (!usersChannel) {
                ...
                guild.channels
                  ...
                  .then(async (ch) => {
                    let role = ch.guild.roles.cache.find(
                      (r) => r.id == options.staffRole
                    );
                    ...
                    let usersCreatedChannel = await guild.channels.cache.find(
                      (ch) => ch.name === mailName.toLowerCase()
                    );
                    ...
                    let deleteRow = new MessageActionRow().addComponents([
                      delButton,
                    ]);
                    ...
                    const openedStaffEmbed = new Discord.MessageEmbed()
                      ...
                      .setDescription(
                        `${options.staffOpenedMessage}\n**User: ${message.author.tag} (${message.author.id})**\n\n ${message.content}`
                      )
                      .setTimestamp()
                      .setColor(options.embedColor);

                    usersCreatedChannel.send({
                      embeds: [openedStaffEmbed],
                      components: [deleteRow],
                    });
                  });
              } else {
                let usersHadChannel = await guild.channels.cache.find(
                  (ch) => ch.name === mailName.toLowerCase()
                );
              ...
              if (message.channel.type === "GUILD_TEXT") {
                ...
                const usertosend = message.guild.members.cache.find(
                  (user) => user.id == message.channel.name
                );
          ...
          client.on("interactionCreate", (interaction) => {
            if (interaction.customId === "close_mail") {
              ...
              if (interaction.customId === "cancle_mail") {
                ...
                let deleteRow2 = new MessageActionRow().addComponents([
                  delButton2,
                ]);
        ...
        interaction.message.channel.send("Ticket creation cancelled.");
